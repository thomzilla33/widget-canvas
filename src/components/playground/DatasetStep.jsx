...
import { X, Plus, Trash2, Database, Table2, PackagePlus, Search, ChevronRight, Check, SlidersHorizontal, ChevronDown, Hash, Calendar, Type as TypeIcon } from 'lucide-react'
import {
  ENTITY_SOURCES, PRESET_DATASETS, COLUMN_TYPES, AGG_FUNCTIONS,
  DATASET_SHAPE, OPERATORS_BY_TYPE, COLUMN_META,
...
const TYPE_LABELS = { string: 'Text', number: 'Number', date: 'Date' }
const TYPE_ICON = { number: Hash, date: Calendar, string: TypeIcon }

const getColMeta = (col) => COLUMN_META[col] ?? { display_name: col, description: null, type: null }
...
// ── Column picker (replaces native <select> for column fields) ────────────────
function ColumnSelect({ value, onChange, columns, placeholder = 'Column…', disabled = false }) {
  ...
  const filtered = columns.filter((col) => {
    ...
    return m.display_name.toLowerCase().includes(q) || col.toLowerCase().includes(q) || (m.description ?? '').toLowerCase().includes(q)
  })
  ...
  return (
    <div ref={wrapRef} className="relative flex-1">
      ...
      <button
        type="button"
        disabled={disabled}
        onClick={() => { if (!disabled) { setOpen((o) => !o); setQuery(''); setHovered(null) } }}
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[200px] overflow-hidden rounded-xl border border-white/10 bg-[#0d0f14] shadow-2xl shadow-black/60">
          ...
          <div className="max-h-44 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <p className="px-3 py-2 text-[11px] text-slate-500">No columns match</p>
            ) : filtered.map((col) => {
              const m    = getColMeta(col)
              ...
              <div className="space-y-2">
                {(config.calculations || []).map((calc, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <ColumnSelect
                      value={calc.column || ''}
                      onChange={(val) => updateCalc(i, 'column', val)}
                      columns={calc.fn === 'count' ? availableColumns : aggNumericColumns}
              ...
              <div className="space-y-2">
                {config.groupers.map((g, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <ColumnSelect
                      value={g.column}
                      onChange={(val) => updateGrouper(i, val)}
