...
function OutputPreviewDrawer({ output, onClose }) {
      ...
      >
        ...
        <div className="flex items-center gap-2 border-t border-gray-100 px-5 py-4 dark:border-white/[0.07]">
          <button
            type="button"
            onClick={escalate}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-gray-200 py-2 text-[12px] font-semibold text-gray-600 transition-colors hover:border-gray-300 hover:bg-gray-50 dark:border-white/[0.1] dark:text-slate-300 dark:hover:bg-white/[0.05]"
          >
            <ArrowUpRight size={13} /> Escalate to a person
          </button>
          <button
            type="button"
            onClick={onClose}
...
export function PendingOutputsCard() {
  ...
  const active   = outputs.filter(o => o.status !== 'advanced')
  const done     = outputs.filter(o => o.status === 'advanced')
  const visible  = [...active, ...done].slice(0, MAX_VISIBLE_OUTPUTS)
  const overflow = outputs.length - visible.length
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`truncate text-xs font-medium ${
                        ...
                        {output.title}
                        {isNew && (
                          <span className="ml-1.5 inline-flex h-1.5 w-1.5 rounded-full bg-aims-blue animate-pulse" aria-hidden="true" />
                        )}
                      </p>
                    ...
                    {/* Right: status chip + authority chip */}
