      ...
      {visible.length === 0 ? (
        ...
        <>
          <ul className="mt-2 -mx-1">
            {shown.map((m) =>
              isErr(m) ? (
                <ErrorRow key={m.id} m={m} retrying={retrying.has(m.id)} onRetry={() => retry(m)} />
              ) : (
                <MessageRow
                  key={m.id}
                  m={m}
    ...
    <li className="group flex items-start gap-2 rounded-md px-2 py-1.5 hover:bg-gray-50 dark:hover:bg-white/5">
      ...
      <button onClick={onOpen} className="min-w-0 flex-1 text-left">
        ...
        <span className="block truncate text-[11px] text-gray-400 dark:text-slate-500">
          {m.from} · {recovered ? 'just now' : m.when}
        </span>
      </button>
...
function ErrorRow({ m, retrying, onRetry }) {
  return (
